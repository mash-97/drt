...
module DRT1
  ...
  def self.get_semester_result(student_id, semester_id, logger=nil, faraday_connection = FARADAY_CONNECTION_GEN.call())
    ...
    response = faraday_connection.get('result', studentId: student_id)
    ...
    if parsed_semester_result.length = 0 then
  ...
  def self.update_semester_result(student_id, semester_id, logger=nil, faraday_connection = FARADAY_CONNECTION_GEN.call())
    ...
    if not student then
      ...
      if not student then
        logger.warning("(update_semester_result) #{student_id}:#{semester_id} [could not find student info, aborting]") if logger
        return nil
      end
    end
    ...
    if not semester_result then
      logger.warning("(update_semester_result) #{student_id}:#{semester_id} [no semester result found, aborting]") if logger
      return nil
    end
    ...
    semester_result.each do |ssr|
      ...
      if tssr
        tssr.update(**ssr)
        logger.info("(update_semester_result) #{student_id}:#{semester_id} [#{ssr[:course_id]}:#{ssr[:course_title]} found in db, updated]") if logger
      else
        tssr = SemesterResult.create(**ssr)
        logger.info("(update_semester_result) #{student_id}:#{semester_id} [#{ssr[:course_id]}:#{ssr[:course_title]} not found in db, created]") if logger
        new_insertion += 1
      end
      updated_semester_result << tssr
    end

    logger.info("(update_semester_result) #{student_id}:#{semester_id} [#{new_insertion} created, #{updated_semester_result.length - new_insertion} updated]") if logger
